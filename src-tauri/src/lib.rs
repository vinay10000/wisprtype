  ...
  tauri::Builder::default()
    .setup(|app| {
      ...
      if let Err(e) = hotkey_manager.register(hotkey) {
        let _ = app.handle().emit(
          "engine-state",
          core::engine::EngineState::Error(format!("Failed to register Win+Space hotkey: {}", e)),
        );
        return Ok(());
      }
      ...
      thread::spawn(move || {
          match core::engine::CoreEngine::new(handle, hotkey_id) {
              Ok(engine) => engine.run(),
              Err(e) => {
                eprintln!("Failed to initialize CoreEngine: {}", e);
                let _ = event_handle.emit(
                  "engine-state",
                  core::engine::EngineState::Error(format!("Engine initialization failed: {}", e)),
                );
              }
          }
      });
